...
export default function VelocityChart({ team = "", employee = "", startDate = "", endDate = "" }) {
  ...
  const { hasStoryPoints, maxPoints } = useMemo(() => {
    ...
    const maxVal = Math.max(...data.map(d => Math.max(d.committed || 0, d.completed || 0)), 1);
  ...
  if (!data || data.length === 0) {
    return (
      <Card className="min-h-[300px] flex justify-center items-center">
        <div className="p-4 text-sm text-gray-500">No velocity data to display for the selected period.</div>
      </Card>
    );
  }
  ...
  if (!hasStoryPoints) {
    return (
      <Card className="min-h-[300px] flex justify-center items-center">
        <div className="p-4 text-sm text-gray-500">No Story Points were found for this period. Velocity chart is hidden.</div>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <div className="text-sm text-gray-500">Story Points Committed vs. Completed</div>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          {data.map((sprint) => {
            const committedPct = Math.round((sprint.committed / maxPoints) * 100);
            const completedPct = Math.round((sprint.completed / maxPoints) * 100);
              ...
              <div key={sprint.name} className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-gray-500">
                    Committed: {sprint.committed} SP • Completed: {sprint.completed} SP
                  </span>
                </div>
                {/* Barra de Puntos Comprometidos */}
                <div className="w-full h-3 rounded-md" style={{ backgroundColor: COLORS.track }}>
                  <div
                    style={{
                      width: `${committedPct}%`,
                      height: "100%",
